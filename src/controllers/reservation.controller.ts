...
import { CreateNewReservationInputType, DeleteReservationInputType } from "../schemas/reservation.schema";
import { userRole } from "../schemas/user.schema";
import { createNewReservation, deleteReservation, getAllReservation } from "../services/reservation.service";
...
export const cancelReservationHandler = async (
    req: Request<{}, {}, DeleteReservationInputType>, res: Response, next: NextFunction
) => {
    try {
        const { reservation_id } = req.body
        const deleteAffected = await deleteReservation(reservation_id)
        return res.send({
            status: success,
